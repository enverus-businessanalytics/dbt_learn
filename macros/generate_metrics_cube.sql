...
{%- if metric_slices|length == 0 and include_overall_total != true %}
  {{ exceptions.raise_compiler_error("Either metrtic_slices or include_overall_total must be defined") }}
{%- endif -%}
...
cte_grouping_sets as (
  ...
  where {{ anchor_date }} between '2014-01-01' and dateadd(year,1,current_date())
    ...
    select
          ...
          {% for _ in metric_slices -%}
            when combination_{{loop.index}}_bit = 0 then concat(
              {%- for dimension in metric_slices[loop.index0] -%}
                ifnull(JSON_EXTRACT_PATH_TEXT(slice_object, 'dim_name'), 'null') {%- if not loop.last -%}, ' x ', {% endif %}
              {%- endfor -%}
            )
          {% endfor -%}
          {% if include_overall_total == true -%}
        ...
        case
          {% for _ in metric_slices -%}
            when combination_{{loop.index}}_bit = 0 then concat(
              {%- for dimension in metric_slices[loop.index0] -%}
                ifnull(JSON_EXTRACT_PATH_TEXT(slice_object, 'dim_value'), 'null') {%- if not loop.last -%}, ' x ', {% endif %}
              {%- endfor -%}
            )
          {% endfor -%}
          {% if include_overall_total == true -%}
